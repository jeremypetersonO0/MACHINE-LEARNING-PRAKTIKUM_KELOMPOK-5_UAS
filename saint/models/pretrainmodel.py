...
class SAINT(nn.Module):
    ...
    def embed_data(self, x_categ, x_cont):
        # offset indeks kategori supaya sesuai posisi di embedding table gabungan
        x_categ = x_categ + self.categories_offset.type_as(x_categ)
        x_categ_enc = self.embeds(x_categ)  # (batch, num_categories, dim)

        n1, n2 = x_cont.shape
        x_cont_enc = torch.empty(n1, n2, self.dim, device=x_cont.device)
        for i in range(self.num_continuous):
            x_cont_enc[:, i, :] = self.simple_MLP[i](x_cont[:, i])
        # (batch, num_continuous, dim)

        return x_categ_enc, x_cont_enc

    def forward(self, x_categ, x_cont):

        x_categ_enc, x_cont_enc = self.embed_data(x_categ, x_cont)
        x = self.transformer(x_categ_enc, x_cont_enc)

        # representasi token pertama (fitur kategori dummy) dipakai sebagai
        # representasi gabungan ala-CLS untuk klasifikasi akhir
        reps = x[:, 0, :]
        y_outs = self.mlpfory(reps)  # (batch, y_dim) -> logit klasifikasi
